...
from IKEAVideo.dataloader.assembly_video import load_annotation, load_video, load_frame, canonicalize_subassembly_parts, find_keyframes, find_subass_frames



class KeyframeDataset(torch.utils.data.Dataset):

    def __init__(self, annotation_file: str, video_dir: str, obj_dir: str, transform, load_into_mem: bool = False, verbose: bool = False, debug: bool = False, return_obj_paths: bool = True, manual_img_dir: str = None):
        """
    ...
    def build_frame_data(self) -> List[Dict]:
        ...
        for category, name, video_url in tqdm(self.cat_name_video_to_frames, desc="Building frame data"):
            ...
            for frame_d in frames:
                ...
                frame_data['manual_img'] = Image.open(os.path.join(self.manual_img_dir, category, name, f'step_{frame_data["step_id"]}.png'))


                if self.load_into_mem:
                    frame_img = load_frame(video, frame_time)
                    frame_data['img'] = frame_img
                    frame_data['manual_img'] = Image.open(
                else:
